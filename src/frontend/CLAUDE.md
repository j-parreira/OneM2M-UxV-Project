...
| S3 primeiros 9 comandos com timeout (cin_create_ms=6–7s) após S2 a 16 msg/s | **Corrigido (2026-06-04)**: `sleep(3)` não chega para drenar a fila TinyDB (~135s necessário). Substituído por `_wait_for_cse_recovery()` — loop que faz GET `/id-in` até RTT < 300ms em ambos: `_run_scenario_1` finally e início de `_run_scenario_2`. |
