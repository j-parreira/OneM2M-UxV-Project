...
| S1 run002: 421/240 entregues, mean latency 96 s | **Corrigido (2026-06-04)**: `_run_scenario_1` agora drena a fila do CSE durante 3 s antes de iniciar a janela de medição (`_S1_DRAIN_S = 3.0`). O CSE TinyDB mantém notificações em fila entre ligações — ao reconectar o Streamlit recebia o backlog da sessão anterior. O drain descarta esses CINs obsoletos sem medir a recuperação do CSE. Configurável em `_S1_DRAIN_S`. Se o backlog for muito grande (após sessão a 16 msg/s), fazer `docker compose restart`. |
...
| S3 WS: 20–30% ACK loss (WS), MQTT 100% | **Corrigido (2026-06-04)**: duas mudanças: (1) drain WS aumentado de 3s para 10s (`_S3_DRAIN_WS`) — o CSE asyncio entrega notificações WS directamente (compete com writes TinyDB), enquanto MQTT delega ao Mosquitto e é mais resiliente; (2) timeout por comando aumentado de 10s para 15s (`_ACK_CMD_TIMEOUT_S`) — notificações WS do CSE asyncio têm maior jitter que MQTT. Diagnóstico: se nos logs "[S3] seq=N ACK timeout" for seguido imediatamente de "[S3] stale ACK seq_cmd=N" no comando seguinte, o ACK chegou depois do deadline → timeout era demasiado curto. |
| S3 WS: ~10% ACK loss residual (seq=2,18,39,40,50,57 em run001) | **Resultado de paper, não bug** (2026-06-04): ACKs genuinamente nunca chegam (sem "[WS] ACK notification seq_cmd=N" nos logs). Não é timeout — é falha de entrega de notificação WS pelo CSE asyncio (~1 em 10 comandos). MQTT tem 0% loss porque usa Mosquitto como buffer assíncrono. Documentar no paper como diferença de fiabilidade WS vs MQTT. |
| Contaminação cross-run após S2 (16 msg/s) | **Procedimento obrigatório** (2026-06-04): após cada run S2, fazer `docker compose restart` antes de qualquer run seguinte. Mecanismo: S2 deixa ~666 notificações em fila no TinyDB; o CSE leva ~136s a limpá-las a 4.9 msg/s; o drain de 3s do orchestrator só remove ~15 entradas. S2→S1 infla `latency_ms` até 90s (medido empiricamente). S2→S2 entrega notificações do run anterior durante o run seguinte (dados inválidos). S2→S3 congestion nos primeiros ~9 cmds (6–7s cin_create_ms). Ver tabela completa em `CLAUDE.md` do projecto. |
