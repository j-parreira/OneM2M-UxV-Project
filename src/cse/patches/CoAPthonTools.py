...
"""	This module provides helper functions to work with CoAPthon.
	...
	Patched for UxV Benchmark (2026-05-31):
	  Added Operation.NOTIFY -> POST to operationsMethodsMap.
	  ACME CSE v2025.11 omits NOTIFY from this map, causing a KeyError when the
	  CSE tries to deliver CoAP notifications (NOTIFY operation) to a coap:// nu URI.
	  oneM2M TS-0010 specifies that NOTIFY maps to POST at the CoAP binding layer.
"""
...
operationsMethodsMap = {
	...
	# NOTIFY maps to POST per oneM2M TS-0010 CoAP binding.
	# Missing from ACME CSE v2025.11 — patched to fix KeyError on notification delivery.
	Operation.NOTIFY: defines.Codes.POST.number,
}
